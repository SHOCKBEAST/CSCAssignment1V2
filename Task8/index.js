...
function getFormattedString(output) {
  ...
  var maxItems = 3;
  // General
    if (output.model.model_version.id === "aa7f35c01e0642fda5cf400f543e7c40") {
  ...
  // Apparel 
    ...
    // Celebrity
    else if (output.model.model_version.id === "2ba4d0b0e53043f38dbbed49e03917b6") {
    ...
    if (data.regions.length === 1) {
      ...
      formattedString = "The people in this picture we are confident in detecting are:<br/>";
        ...
        for (var i = 0; i < items.length; i++) {
            var item = data.regions[i].data.concepts;
            for (var i = 0; i < maxItems; i++){
                formattedString += "- " + item[i].name + " at a " + (Math.round(item[i].value * 10000) / 100) + "% probability<br/>";
            };
    }
  }
  // Color
